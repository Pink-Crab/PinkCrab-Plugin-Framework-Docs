...
There are 3 types of event which can be extended within your Application. These allow you to dispatch custom events to the [Event Queue](queue-service) to be handled by your Application.

Each of these events comes with its own Abstract Class which must be extended to create your own custom event.

All events must have a matching [`Event_Listener`](event-listener) which will be triggered when the event is processed by the queue.

## Async Event

As the name implies, these events are triggered as soon as possible by the [Event Queue](queue-service). They are not delayed nor recurring.
...
This event will be triggered 1 day after it is dispatched to the [Event Queue](queue-service).
...
The event will be triggered 1 day after it is dispatched to the [Event Queue](queue-service), and then every 1 day after that.


# Dispatching Events

This can then be dispatched to the [Event Queue](queue-service) by either injecting the `Queue_Service` into your Controller or by using the `Queue::dispatch()` helper function.
