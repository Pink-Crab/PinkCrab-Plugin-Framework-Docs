...
For more details on setting up the module or creating custom drivers, please [see the Module Docs](queue-driver) for more details.
...
* [`PinkCrab\Queue\Event\Async_Event`](events#async-event) - A simple event which will be run as soon as the queue is processed.
* [`PinkCrab\Queue\Event\Delayed_Event`](events#delayed-event) - A simple event which will be run after a delay.
* [`PinkCrab\Queue\Event\Recurring_Event`](events#recurring-event) - A simple event which will be run after a delay, and then again after a delay.

Please [see the Events Docs](events) for more details.
...
You can read more about the [`Queue_Service` here](queue-service).
...
For more details on the `Abstract_Listener` class, please [see the docs here](event-listener).
